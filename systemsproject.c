...
void fractionToBinary(char fractionBinary[], float fractionPartOfFloat){
 	int i = 0;
 	while(fractionPartOfFloat != 1) { //fraction to binary
		if (fractionPartOfFloat == 0) {
			strcpy(fractionBinary, "0");
			fractionBinary[1] = '\0';
			break;
		}else if(fractionPartOfFloat > 1) fractionPartOfFloat -= 1;	 	
		
		fractionPartOfFloat *= 2;
		if(fractionPartOfFloat < 1) fractionBinary[i++] = '0';		 
		else if(fractionPartOfFloat>1)	fractionBinary[i++] = '1';		 
		else {
			fractionBinary[i++] = '1';
			break;
		}		 
